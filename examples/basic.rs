...
impl AvCaptureVideoDataOutputSampleBufferDelegate for CameraM {
	...
	fn init(delegate: WrappedAvCaptureVideoDataOutputSampleBufferDelegate) -> CameraM {
		...
		let queue = unsafe {
			...
			// freed too early. This happens because the pointer returned by `as_ptr` does not carry 
			// any lifetime information and the string is deallocated immediately after 
